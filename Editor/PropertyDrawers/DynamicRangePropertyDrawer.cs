    ...
    public class DynamicRangeDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (parentObject != null)
            {
                ...
                if (!rangeAttribute.ShowRangeValues)
                {
                    // Draw the slider
                    DrawSlider(position, property, range);
                    return;
                }
                ...
                float sliderWidth =
                    position.width - nameLabelWidth - (rangeValueWidth * 2) - (sliderPadding * 2);
                ...
                Rect minRangeRect = new Rect(
                    nameLabelRect.x + nameLabelWidth,
                    position.y,
                    rangeValueWidth,
                    position.height
                );
                Rect sliderRect = new Rect(
                    minRangeRect.x + rangeValueWidth + sliderPadding,
                    position.y,
                    sliderWidth,
                    position.height
                );
                Rect maxRangeRect = new Rect(
                    sliderRect.x + sliderWidth + sliderPadding,
                    position.y,
                    rangeValueWidth,
                    position.height
                );
        ...
        void DrawSliderWithoutName(Rect sliderRect, SerializedProperty property, Vector2 range)
        {
            if (property.propertyType == SerializedPropertyType.Float)
            {
                property.floatValue = EditorGUI.Slider(
        ...
        private Vector2 GetVector2Value(string fieldName, object targetObject)
        {
            ...
            Debug.LogError(
                $"DynamicRangeAttribute: Could not find Vector2 field or property '{fieldName}' on '{targetObject.GetType()}'."
            );
            return Vector2.zero;
        }
        ...
        private object GetParentObject(string path, object obj)
        {
            ...
            foreach (var field in fields.Take(fields.Length - 1))
            {
                ...
                if (fieldInfo != null)
                {
                    ...
                    {
                        Debug.LogError(
                            $"DynamicRangeAttribute: Field '{field}' is null on object of type '{type}'."
                        );
                        return null;
                    }
                }
                ...
                {
                    Debug.LogError(
                        $"DynamicRangeAttribute: Could not find field '{field}' on object of type '{type}'."
                    );
                    return null;
                }
            }
