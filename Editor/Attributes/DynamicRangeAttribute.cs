...
namespace Darklight.UnityExt.Editor
{
    ...
    public class DynamicRangeDrawer : PropertyDrawer
    {
        ...
        void DrawSlider(Rect sliderRect, SerializedProperty property, Vector2 range)
        {
            ...
            {
                property.floatValue = EditorGUI.Slider(sliderRect, property.name, property.floatValue, range.x, range.y);
            }
            else if (property.propertyType == SerializedPropertyType.Integer)
            {
                property.intValue = EditorGUI.IntSlider(sliderRect, property.name, property.intValue, (int)range.x, (int)range.y);
            }
            else
            {
        ...
        void DrawSliderWithoutName(Rect sliderRect, SerializedProperty property, Vector2 range)
        {
            ...
            {
                property.intValue = EditorGUI.IntSlider(sliderRect, property.intValue, (int)range.x, (int)range.y);
            }
            else
            {
                EditorGUI.LabelField(sliderRect, "Use DynamicRange with float or int.");
