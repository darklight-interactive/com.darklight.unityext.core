...
public class MatrixWeightComponentPreset : ScriptableLibrary<Vector2Int, int, Library<Vector2Int, int>>
{
    [SerializeField]
...
#if UNITY_EDITOR
    [CustomEditor(typeof(MatrixWeightComponentPreset))]
    public class Grid2D_WeightDataObjectCustomEditor : UnityEditor.Editor
    {
        SerializedObject _serializedObject;
        MatrixWeightComponentPreset _script;
        private void OnEnable()
        {
            _serializedObject = new SerializedObject(target);
            _script = (MatrixWeightComponentPreset)target;
        }
