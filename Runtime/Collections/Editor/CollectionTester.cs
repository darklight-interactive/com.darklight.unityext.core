...
namespace Darklight.Collections.Editor
{
    ...
    public class LibraryTester : MonoBehaviour
    {
        ...
        [SerializeField]
        private CollectionLibrary<ScriptableObject> _scriptableObjectLibrary = new();

        [SerializeField]
        private CollectionDictionary<string, ScriptableObject> _scriptableObjectDictionary = new();

        [Header("String Libraries")]
        [SerializeField]
        private CollectionLibrary<string> _stringLibrary = new();

        [SerializeField]
        private CollectionDictionary<int, string> _indexedStringDictionary = new();

        [Header("Primitive Libraries")]
        [SerializeField]
        private CollectionLibrary<int> _intLibrary = new();

        [SerializeField]
        private CollectionLibrary<float> _floatLibrary = new();

        [SerializeField]
        private CollectionLibrary<bool> _boolLibrary = new();

        [Header("Vector Libraries")]
        [SerializeField]
        private CollectionLibrary<Vector2> _vector2Library = new();

        [SerializeField]
        private CollectionLibrary<Vector3> _vector3Library = new();

        [SerializeField]
        private CollectionDictionary<string, Vector3> _namedVector3Dictionary = new();

        [Header("Unity Type Libraries")]
        [SerializeField]
        private CollectionLibrary<Color> _colorLibrary = new();

        [SerializeField]
        private CollectionLibrary<Quaternion> _rotationLibrary = new();

        [SerializeField]
        private CollectionLibrary<AnimationCurve> _curveLibrary = new();

        [Header("Component Libraries")]
        [SerializeField]
        private CollectionLibrary<Transform> _transformLibrary = new();

        [SerializeField]
        private CollectionLibrary<Rigidbody> _rigidbodyLibrary = new();

        [SerializeField]
        private CollectionDictionary<string, MonoBehaviour> _behaviorDictionary = new();

        [Header("Array Libraries")]
        [SerializeField]
        private CollectionLibrary<string[]> _stringArrayLibrary = new();

        [SerializeField]
        private CollectionLibrary<int[]> _intArrayLibrary = new();

        [SerializeField]
        private CollectionLibrary<Vector3[]> _vector3ArrayLibrary = new();

        [Header("List Libraries")]
        [SerializeField]
        private CollectionLibrary<List<string>> _stringListLibrary = new();

        [SerializeField]
        private CollectionLibrary<List<ScriptableObject>> _scriptableObjectListLibrary = new();

        [SerializeField]
        private CollectionDictionary<string, List<string>> _stringListDictionary = new();

        [Header("Complex Type Libraries")]
        [SerializeField]
        private CollectionLibrary<UnityEvent> _eventLibrary = new();

        [SerializeField]
        private CollectionDictionary<string, Dictionary<string, object>> _metadataDictionary =
            new();
        ...
        private void PopulateTestData()
        {
            ...
            _stringListLibrary.Add(
                new CollectionItem<List<string>>(
                    0,
                    new List<string> { "List1-A", "List1-B", "List1-C" }
                )
            );
            _stringListLibrary.Add(
                new CollectionItem<List<string>>(
                    1,
                    new List<string> { "List2-A", "List2-B", "List2-C" }
                )
            );
            _stringListLibrary.Add(
                new CollectionItem<List<string>>(
                    2,
                    new List<string> { "List3-A", "List3-B", "List3-C" }
                )
            );

            _stringListDictionary.Add(
                "List1",
                new List<string> { "Dict1-A", "Dict1-B", "Dict1-C" }
            );
            _stringListDictionary.Add(
                "List2",
                new List<string> { "Dict2-A", "Dict2-B", "Dict2-C" }
            );
            _stringListDictionary.Add(
                "List3",
                new List<string> { "Dict3-A", "Dict3-B", "Dict3-C" }
            );

            // Populate Complex Type Libraries
            _metadataDictionary.Add(
                "Meta1",
